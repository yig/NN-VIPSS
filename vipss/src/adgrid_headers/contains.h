...
bool is_separating_plane(std::span<T> pts, std::span<T> query_point, size_t i,
    ...
    const auto ori_query = orient3dT(pa, pb, pc, pd);
    if (ori_query == 0) {
    ...
    for (size_t l = 0; l < num_pts; l++) {
        ...
        T* pe = pts.data() + DIM * l;
        const auto ori = orient3dT(pa, pb, pc, pe);
        if (ori * ori_query > 0) {
            return false;
        }
    }
...
bool is_separating_plane(std::span<T> pts, std::span<T> query_point, size_t i,
    ...
    const auto ori_query = orient2dT(pa, pb, pc);
    if (ori_query == 0) {
        // Query point is coplanar with the triangle formed by i, j, k.
        // It is contained in the convex hull by definition.
        return false;
    ...
    for (size_t l = 0; l < num_pts; l++) {
        ...
        T* pd = pts.data() + DIM * l;
        const auto ori = orient2dT(pa, pb, pd);
        if (ori * ori_query > 0) {
            return false;
        }
    }
