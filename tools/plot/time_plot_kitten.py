...
vipss_time_list = [0.000167259, 0.000326565, 0.000492989, 0.000659512,0.000815157, 0.00096466, 0.00112545,0.00128707, 0.00144556, 0.00163293]
...
for name in file_names:
    stats_path = os.path.join(stats_dir, 'kitten_' + name + '_time_stats.txt') 
...
for octree_level in octree_depth:
    ...
    for name in file_names:
        stats_path = os.path.join(octree_out_dir, 'kitten_' + name + '_time_stats.txt') 
        with open(stats_path, mode='r') as file:
            reader = csv.reader(file)
            next(reader)  # Skip the header if the file has one
            for row in reader:
...
for i in range(len(vipss_time_list)):
    vipss_time_list[i] *= evaluate_times[i] 
...
plt.title("Surface time with different level of octree")
plt.legend()
